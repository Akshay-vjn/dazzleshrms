...
import '../../approvals/presentation/widgets/approval_sheet.dart';
import '../../approvals/presentation/widgets/rejection_sheet.dart';
import '../data/models/employee_pending_leave_model.dart';
import '../data/provider/employee_leave_provider.dart';

class EmployeePendingLeavesScreen extends ConsumerStatefulWidget {
  final int employeeId;
  final String employeeName;

  const EmployeePendingLeavesScreen({
    super.key,
  ...
  @override
  ConsumerState<EmployeePendingLeavesScreen> createState() => _EmployeePendingLeavesScreenState();
}

class _EmployeePendingLeavesScreenState extends ConsumerState<EmployeePendingLeavesScreen> {
  final int _limit = 10;
  int _currentPage = 1;
  ...
  Future<void> _refresh() async {
    _currentPage = 1;
    await ref.read(employeePendingLeavesProvider(widget.employeeId).notifier).loadPendingLeaves(
          page: _currentPage,
          limit: _limit,
        );
  }
  ...
  Widget build(BuildContext context) {
    final pendingState = ref.watch(employeePendingLeavesProvider(widget.employeeId));
